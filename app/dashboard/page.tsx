...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // Detector inteligente que sincroniza com o menu lateral em tempo real
    const detectTheme = () => {
      const html = document.documentElement
      const body = document.body
      
      // 1. Verifica classes de tema
      ...
      // 2. Verifica preferência salva no localStorage do menu
      const storedTheme = localStorage.getItem('theme') || localStorage.getItem('nutrisaas-tema')
      if (storedTheme === 'dark') {
        setIsDark(true)
        return
      }
      ...
      // 3. Analisa a cor de fundo real renderizada pelo layout pai
    ...
    // Ouve alterações no DOM, cliques no menu de tema e eventos de storage
    ...
    window.addEventListener('click', () => setTimeout(detectTheme, 50))
    const interval = setInterval(detectTheme, 400)
    ...
    return () => {
      ...
      window.removeEventListener('click', detectTheme)
  ...
  // Estilos aplicados de forma direta para garantia absoluta
  const cardStyle = {
  ...
  const textMutedStyle = { color: isDark ? '#94a3b8' : '#64748b' }
  const borderDividerStyle = { borderColor: isDark ? '#1e293b' : '#e2e8f0' }
  ...
  if (isLogado) {
    return (
      <div className="space-y-6 animate-in fade-in duration-500 relative min-h-screen pb-16">
        ...
        {/* Cabeçalho */}
        <div className="flex items-center justify-between pb-4 border-b transition-colors duration-300" style={borderDividerStyle}>
          <div>
            <h1 className="text-2xl font-bold text-emerald-500 tracking-tight">
              Bem-vindo(a), Dr(a). {nomeUsuario}
            </h1>
            <p className="text-xs mt-0.5 transition-colors duration-300" style={textMutedStyle}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-5 rounded-2xl border shadow-md flex flex-col gap-1 hover:border-emerald-500/40 transition-all duration-300" style={cardStyle}>
            <span className="text-xs font-bold uppercase tracking-wider" style={textMutedStyle}>Total de Pacientes</span>
            <span className="text-3xl font-extrabold text-emerald-500">0</span>
          </div>
          <div className="p-5 rounded-2xl border shadow-md flex flex-col gap-1 hover:border-sky-500/40 transition-all duration-300" style={cardStyle}>
            <span className="text-xs font-bold uppercase tracking-wider" style={textMutedStyle}>Dietas Ativas</span>
            <span className="text-3xl font-extrabold text-sky-500">0</span>
          </div>
          <div className="p-5 rounded-2xl border shadow-md flex flex-col gap-1 hover:border-amber-500/40 transition-all duration-300" style={cardStyle}>
            <span className="text-xs font-bold uppercase tracking-wider" style={textMutedStyle}>Consultas Hoje</span>
            <span className="text-3xl font-extrabold text-amber-500">{atendimentos.length}</span>
          </div>
          <div className="p-5 rounded-2xl border shadow-md flex flex-col gap-1 hover:border-red-500/40 transition-all duration-300" style={cardStyle}>
            <span className="text-xs font-bold uppercase tracking-wider" style={textMutedStyle}>Avisos / Retornos</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 rounded-2xl border shadow-md overflow-hidden flex flex-col transition-all duration-300" style={cardStyle}>
            <div className="p-5 border-b flex justify-between items-center" style={borderDividerStyle}>
            ...
            <div className="p-5 space-y-3 flex-1">
              {atendimentos.length === 0 ? (
                <div className="p-8 text-center border border-dashed rounded-xl" style={borderDividerStyle}>
                  <p className="text-sm" style={textMutedStyle}>Nenhum atendimento agendado para hoje.</p>
                  <button 
                ...
                atendimentos.map((paciente) => (
                  <div key={paciente.id} className="flex items-center justify-between p-4 rounded-xl border transition-all" style={itemStyle}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[11px]" style={textMutedStyle}>{paciente.tipo}</p>
                      </div>
                    </div>
          ...
          {/* Ações Rápidas */}
          <div className="rounded-2xl border shadow-md flex flex-col transition-all duration-300" style={cardStyle}>
            <div className="p-5 border-b" style={borderDividerStyle}>
            ...
            <div className="p-5 flex flex-col gap-3">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[10px]" style={textMutedStyle}>Marcar atendimento no sistema</p>
                </div>
              </button>

              <Link href="/dashboard/pacientes" className="w-full text-left p-4 rounded-xl border hover:border-emerald-500/50 transition-all flex items-center gap-4 group" style={itemStyle}>
                <span className="text-2xl group-hover:scale-110 transition-transform">👥</span>
                <div>
                  <p className="text-sm font-bold">Novo Paciente</p>
                  <p className="text-[10px]" style={textMutedStyle}>Cadastrar prontuário</p>
                </div>
              </Link>
              
              <Link href="/dashboard/dietas" className="w-full text-left p-4 rounded-xl border hover:border-sky-500/50 transition-all flex items-center gap-4 group" style={itemStyle}>
                <span className="text-2xl group-hover:scale-110 transition-transform">🥗</span>
                <div>
                  <p className="text-sm font-bold">Montar Dieta</p>
                  <p className="text-[10px]" style={textMutedStyle}>Criar plano alimentar</p>
                </div>
              </Link>

              <Link href="/dashboard/anamnese" className="w-full text-left p-4 rounded-xl border hover:border-amber-500/50 transition-all flex items-center gap-4 group" style={itemStyle}>
                <span className="text-2xl group-hover:scale-110 transition-transform">📄</span>
                <div>
                  <p className="text-sm font-bold">Nova Anamnese</p>
                  <p className="text-[10px]" style={textMutedStyle}>Registrar avaliação clínica</p>
                </div>
              </Link>
            </div>
          </div>
        ...
        {modalAgendarAberto && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-md rounded-2xl border p-6 shadow-2xl space-y-4" style={cardStyle}>
              <div className="flex justify-between items-center border-b pb-3" style={borderDividerStyle}>
                <h3 className="text-lg font-bold text-emerald-500">Agendar Consulta</h3>
                <button onClick={() => setModalAgendarAberto(false)} style={textMutedStyle}>✕</button>
              </div>

              <form onSubmit={handleAgendarConsulta} className="space-y-4">
                <div>
                  <label className="block text-xs mb-1" style={textMutedStyle}>Nome do Paciente</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs mb-1" style={textMutedStyle}>Tipo de Consulta</label>
                  <select
                    value={novoTipo}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs mb-1" style={textMutedStyle}>Data</label>
                    <input
                      type="date"
                      value={novaData}
                  ...
                  <div>
                    <label className="block text-xs mb-1" style={textMutedStyle}>Horário</label>
                    <input
                      type="time"
                      value={novoHorario}
  ...
  return (
    <div className="flex items-center justify-center min-h-[75vh]">
      <div className="w-full max-w-md rounded-2xl border p-8 shadow-2xl backdrop-blur-sm transition-all duration-300" style={cardStyle}>
        <div className="text-center mb-8">
          <h1 className="text-3xl font-extrabold text-emerald-500 mb-1 tracking-tight">NutriSaaS</h1>
          <p className="text-xs" style={textMutedStyle}>Acesse o seu painel de gestão nutricional</p>
        </div>

        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            <label className="mb-1.5 block text-xs" style={textMutedStyle}>E-mail Profissional</label>
            <input
              type="email"
              required
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              <label className="block text-xs" style={textMutedStyle}>Senha de Acesso</label>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            />
            <label htmlFor="manter" className="text-xs cursor-pointer" style={textMutedStyle}>
